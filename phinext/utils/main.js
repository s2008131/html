...
Utils.lazyload(function() {
	...
	const w = `
	模拟器原作者/Original Author：<a style="text-decoration:underline"target="_blank"href="//space.bilibili.com/274753872">${d}</a><br>
	新UI开发与设计/New UI Design：<a style="text-decoration:underline"target="_blank"href="//space.bilibili.com/544365132">kev1nweng</a>,
	<a style="text-decoration:underline"target="_blank"href="//space.bilibili.com/295247346">scar922</a>`;
	//if (!location.search) location.search = Date.now();
	for (const i of document.querySelectorAll(".title")) i.innerHTML = `${_i[0]}&nbsp;-&nbsp;版本&nbsp;${_i[1].join('.')}`;
	for (const i of document.querySelectorAll(".info")) i.innerHTML = `${w}<br><br>最后更新于2023年2月18日`;//r
